...
Resource::~Resource()
{
    // Must remove from global state tracker BEFORE the ComPtr releases the resource.
    // If we don't, ms_GlobalResourceState holds a dangling ID3D12Resource* which
    // causes an access violation in FlushPendingResourceBarriers on the next frame.
    if (m_d3d12Resource)
    {
