// * RootSignature = GPU着色器的"函数签名",告诉GPU: 你的shader需要什么资源,从哪里取,取多少

// * Root Signature
// * ├── Root Parameter [0]   ← 三选一：
// * │   ├── Root Constant        (直接内嵌32-bit值,最快, <=64个DWORD)
// * │   ├── Root Descriptor      (直接内嵌GPU虚拟地址,仅CBV/SRV/UAV)
// * │   └── Descriptor Table     (指向DescriptorHeap中一段区域,最灵活)
// * │       ├── Range [0]: CBV  b0~b3
// * │       └── Range [1]: SRV  t0~t7
// * ├── Root Parameter [1]
// * └── Static Samplers []   (烘焙进签名,不占DescriptorHeap空间)
