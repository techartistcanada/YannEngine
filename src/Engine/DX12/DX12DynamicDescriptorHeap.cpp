    ...
    , m_NumFreeHandles(0)
{
	...
	m_DescriptorHandleIncrementSize = DX12Device::GetInst()->GetDescriptorHandleIncrementSize(heapType);
...
void DynamicDescriptorHeap::ParseRootSignature(const RootSignature& rootSignature)
{
    ...
    while (_BitScanForward(&rootIndex, descriptorTableBitMask) && rootIndex < rootSignatureDesc.NumParameters)
    {
        ...
        // ! NOTE:
        // Pick the correct null descriptor based on the table's range type (SRV vs UAV).
        // CopyDescriptors validates that descriptor types match the declared range type.
        const auto& param = rootSignatureDesc.pParameters[rootIndex];
        D3D12_CPU_DESCRIPTOR_HANDLE nullDescriptor;
        if (param.ParameterType == D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE
            && param.DescriptorTable.NumDescriptorRanges > 0
            && param.DescriptorTable.pDescriptorRanges[0].RangeType == D3D12_DESCRIPTOR_RANGE_TYPE_UAV)
        {
            nullDescriptor = DX12Device::GetInst()->GetNullUAV();
        }
        else
        {
            nullDescriptor = DX12Device::GetInst()->GetNullSRV();
        }

        // Fill all slots with the appropriate null descriptor so CopyDescriptors
        // never encounters an invalid {0} handle for unbound slots.
        for (uint32_t i = 0; i < numDescriptors; ++i)
        {
            m_DescriptorHandleCache[currentOffset + i] = nullDescriptor;
        }
...
void DynamicDescriptorHeap::CommitStagedDescriptors(DX12CommandList& commandList, std::function<void(ID3D12GraphicsCommandList*, UINT, D3D12_GPU_DESCRIPTOR_HANDLE)> setFunc)
{
...
void DynamicDescriptorHeap::CommitStagedDescriptorsForDraw(DX12CommandList& commandList)
{
    CommitStagedDescriptors(commandList, &ID3D12GraphicsCommandList::SetGraphicsRootDescriptorTable);
}

void DynamicDescriptorHeap::CommitStagedDescriptorsForDispatch(DX12CommandList& commandList)
{
    CommitStagedDescriptors(commandList, &ID3D12GraphicsCommandList::SetComputeRootDescriptorTable);
}

D3D12_GPU_DESCRIPTOR_HANDLE DynamicDescriptorHeap::CopyDescriptor(DX12CommandList& comandList, D3D12_CPU_DESCRIPTOR_HANDLE cpuDescriptor)
{
